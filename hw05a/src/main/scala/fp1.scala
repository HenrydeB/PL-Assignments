...
object fp1:
  ...
  val factTest: List[Int] =
    val lst = List(1,2,3,4,5) //need to log
    lst.map(fact)
  ...
  def fib(n: Int): Int = //look into the other ways he did this
    if n <= 1 then
      n
    else
      fib(n-1)+ fib(n-2)
  ...
  def maxList(xs: List[Int]): Int =
    xs match{
      ...
      case head::Nil => head
      case head::tail =>
        val nextMax = maxList(tail)
        if head > nextMax then
          head
        else
          nextMax
    }
  ...
  def maxTailAux(xs: List[Int], accumulator: Int): Int =
    xs match {
      case Nil => accumulator
      case head::tail if head > accumulator => maxTailAux(tail, head)
      case _ :: tail => maxTailAux(xs.tail, accumulator)
    }
  ...
  def otpu(start: Int, end: Int): List[Int] =
    (start >= end) match
      case true => start :: otpu(start-1, end)
      case false => List()