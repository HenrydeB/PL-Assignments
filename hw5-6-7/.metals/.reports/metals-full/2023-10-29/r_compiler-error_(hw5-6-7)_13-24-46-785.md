...
object fp3:
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    def helper(counter: Int, x:X ,xs: List[X]): List[(Int, X)] = {
      xs match 
        case Nil => if x == Nil then List() else (x,i) :: List() //if x and the list are null then we just return an empty list, otherwise we want x and i to be in there
        case a::as => if a == x then helper(counter+1, a, as) else helper(1, x, as)
    }
    xs m@@
