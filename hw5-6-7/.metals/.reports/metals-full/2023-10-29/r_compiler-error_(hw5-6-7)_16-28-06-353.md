file:///C:/Users/henry/source/repos/ProgrammingLanguages/hw5-6-7/src/main/scala/fp3.scala
...
action parameters:
offset: 7991
uri: file:///C:/Users/henry/source/repos/ProgrammingLanguages/hw5-6-7/src/main/scala/fp3.scala
text:
```scala
  // GENERATED
/* INSTRUCTIONS
...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean ={
    xs match{
      ...
      case x::Nil => return true; //covers case where we are at the end of the list
      case x::xs => if x == xs.head then allEqual(xs) else return false
    }
  ...
  def stringLengths(xs: List[String]): List[(String, Int)] =
    xs.map(x => (x, x.length)) //map lambda expression that spreads each string and the string's length
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ys match 
      case Nil => Nil
      case y::ys => if x == y then delete1(y, ys) else y::delete1(x,ys)
      //x == y then we can "shorten" the list, otherwise we move delete down the list
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    def helper(counter: Int, x:X ,xs: List[X]): List[(Int, X)] = {
      xs match 
        case Nil => if x == Nil then List() else (counter,x):: List() //if x and the list are null then we just return an empty list, otherwise we want x and i to be in there
        case a::as => if a == x then helper(counter+1, x, as) else (counter,x)::helper(1, a, as) //similar workflow to previous remove dupes but with counter
    }
    if xs.length > 0 then helper(1, xs.head, xs.tail) else List()
  ...
  if n <= 0 then 
    (List(), xs)   //I don't like this syntax, consider refactoring
  else
    xs match 
      case Nil => (List(),List())
      case (head::tail) => 
        val (left, right) = splitAt(n-1, tail) //count the index target down
        (head :: left, right)
