...
object fp3:
  ...
  def removeDupes1_2[X](xs: List[X]): List[X] =
    xs.foldRight(Nil)((y:X, ss:List[X]) => 
      ss match
        case r:: _ if r== y => ss 
        case _        => y::ss
    )
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    def helper(counter: Int, x:X ,xs: List[X]): List[(Int, X)] = {
      xs match 
        ...
        case a::as => if a == x then 
          helper(counter+1, x, as) 
          else 
            (counter,x)::helper(1, a, as) //similar workflow to previous remove dupes but with counter
    }
  ...
  if n <= 0 then 
    (List(), xs)   //handle if n is less than or equal to zero. There's gotta be a better way to do this
  else
    xs match 
      case Nil => (List(),List())
      case (head::tail) => 
  ...
  def allDistinct(xs: List[Int]): Boolean =
    xs match
      case Nil => true
      case head::tail => if member(head, tail) then false else allDistinct(tail)
